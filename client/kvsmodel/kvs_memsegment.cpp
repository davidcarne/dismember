...
KVS_MemSegment::KVS_MemSegment(sp_I_KVS_attributes attribs) : I_KVS_attribproxy(attribs)
 : m_base(base), m_len(len), m_data(NULL), m_name(name)
{
	m_name = I_KVS_attribproxy::getAttrib("name");
	
	init(buffer, flength);
}

KVS_MemSegment::KVS_MemSegment(sp_I_KVS_attributes attribs, paddr_t base, psize_t len, void * buffer, psize_t flength, const std::string & name)
: I_KVS_attribproxy(attribs), m_base(base), m_len(len), m_data(NULL), m_name(name)
{
	init(buffer, flength);
}


static sp_KVS_MemSegment createKVS_MemSegment(sp_I_KVS_node noderef)
{
	sp_KVS_MemSegment spr(new KVS_MemSegment(noderef));
	spr->m_me = spr;
	return spr; 
}
