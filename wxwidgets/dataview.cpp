...
void MemoryLayoutView::OnPaint(wxPaintEvent& WXUNUSED(event))
{
	...
	for (MemSegmentManager::memseglist_ci mi = m_trace.memsegs_begin(); mi != m_trace.memsegs_end(); mi++)
	{
	...
	MemSegmentManager::memseglist_ci mi = m_trace.memsegs_begin();
...
void MemoryLayoutView::OnClick(wxMouseEvent& m)
{
	...
	((DocumentWindow *)m_parent)->getCodeListingView()->jumpToAddr(m_pixel_addr_map[m.GetX()]);
	m.Skip();
}
